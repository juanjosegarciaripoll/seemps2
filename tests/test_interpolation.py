...
from analysis.space import Space
from analysis.interpolation import *
from .tools import *

class TestInterpolation(TestCase):
    
    strategy = DEFAULT_STRATEGY.replace(simplify=True)
    ...
    def test_fourier_interpolation_1D(self):
        for n in range(3,8):
            ...
            sol_N = gaussian(r_N)
            sol_N_mps = MPS.from_vector(sol_N, [2]*sum(qubits_per_dimension), normalize=False)
            m = n+2
            M = 2**m
            sol_int = fourier_interpolation_vector_1D(sol_N, M)
            sol_int /= np.linalg.norm(sol_int)
            sol_int_mps, _ = fourier_interpolation_1D(sol_N_mps, space, n, m, dim=0, strategy=self.strategy)
    ...
    def test_fourier_interpolation(self):
        for n in range(3,8):
            ...
            sol_int /= np.linalg.norm(sol_int)
            sol_int_mps = fourier_interpolation(sol_N_mps, space, qubits_per_dimension, qubits_per_dimension_M, strategy=self.strategy)
    ...
    def test_finite_differences_interpolation_1D(self):
        for n in range(3,8):
            ...
            sol_int /= np.linalg.norm(sol_int)
            sol_int_mps, _ = finite_differences_interpolation_1D(sol_N_mps, space, strategy=self.strategy)
            self.assertSimilar(sol_int.flatten(),sol_int_mps)
    ...
    def test_finite_differences_interpolation(self):
        for n in range(3,8):
            ...
            sol_int /= np.linalg.norm(sol_int)
            sol_int_mps = finite_differences_interpolation(sol_N_mps, space, strategy=self.strategy)
            self.assertSimilar(sol_int.flatten(),sol_int_mps)