    ...
    def test_random_mps_uses_rng(self):
        ...
        self.assertTrue(
            all(
                np.all(A == B)
                for A, B in zip(
                    random_mps(2, 10, rng=rng1), random_mps(2, 10, rng=rng2)
                )
            )
        )
