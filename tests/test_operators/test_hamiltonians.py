...
class TestConstantNNHamiltonian(TestCase):
    def assertSimilarMatrix(
        self, H: NNHamiltonian, M: sp.spmatrix | np.ndarray
    ) -> None:
    ...
    def test_adding_local_term_to_sides_adds_them_to_interaction(self):
        ...
        self.assertSimilar(H2.interaction_term(0), sp.kron(σx, i2))
        self.assertSimilarMatrix(H2, sp.kron(σx, i2))

        H2 = ConstantNNHamiltonian(2, 2)
        H2.add_local_term(1, σy)
        self.assertSimilar(H2.interaction_term(0), sp.kron(i2, σy))
        self.assertSimilarMatrix(H2, sp.kron(i2, σy))

    def test_adding_local_term_inside_splits_in_two_interactions(self):
        H2 = ConstantNNHamiltonian(3, 2)
        H2.add_local_term(1, σx)
        self.assertSimilar(H2.interaction_term(0), sp.kron(i2, 0.5 * σx))
        self.assertSimilar(H2.interaction_term(1), sp.kron(0.5 * σx, i2))
        self.assertSimilarMatrix(H2, sp.kron(i2, sp.kron(σx, i2)))

    def test_adding_product_interaction_kronecker_product_matrix(self):
        H2 = ConstantNNHamiltonian(2, 2)
        H2.add_interaction_term(0, σx, σy)
        self.assertSimilar(H2.interaction_term(0), sp.kron(σx, σy))
        self.assertSimilarMatrix(H2, sp.kron(σx, σy))

    def test_add_interaction_with_one_term_stores_matrix(self):
        H2 = ConstantNNHamiltonian(2, 2)
        H2.add_interaction_term(0, sp.kron(σx, σy))
        self.assertSimilar(H2.interaction_term(0), sp.kron(σx, σy))
        self.assertSimilarMatrix(H2, sp.kron(σx, σy))
    ...
    def test_local_non_uniform_Hamiltonian(self):
        ...
        self.assertSimilarMatrix(H2, sp.kron(-0.5 * σx, i2) + sp.kron(i2, 1.2 * σy))
        ...
        self.assertSimilarMatrix(
            H3,
            sp.kron(-0.5 * σx, sp.kron(i2, i2))
            + sp.kron(i2, sp.kron(1.2 * σy, i2))
            + sp.kron(i2, sp.kron(i2, 0.8 * σz)),
        )

    def test_adding_local_term_with_different_dimensions(self):
        H2 = ConstantNNHamiltonian(2, [3, 2])
        H2.add_local_term(0, Sx)
        self.assertSimilar(H2.interaction_term(0), sp.kron(Sx, i2))
        self.assertSimilarMatrix(H2, sp.kron(Sx, i2))

        H2 = ConstantNNHamiltonian(2, [2, 3])
        H2.add_local_term(1, Sy)
        self.assertSimilar(H2.interaction_term(0), sp.kron(i2, Sy))
        self.assertSimilarMatrix(H2, sp.kron(i2, Sy))

        H2 = ConstantNNHamiltonian(2, [2, 3])
        H2.add_local_term(0, σx)
        H2.add_local_term(1, Sy)
        self.assertSimilar(H2.interaction_term(0), sp.kron(σx, i3) + sp.kron(i2, Sy))
        self.assertSimilarMatrix(H2, sp.kron(σx, i3) + sp.kron(i2, Sy))
