...
def maxvol(G, i_physical, i_sweep, maxvol_options, ltr):
    """
    Maxvol algorithm.
    ...
    elif maxvol_options["max_rank_change"] == 0:  # Force square maxvol
        i_maxvol, Q_maxvol = maxvol_sqr(Q, maxvol_options)
    else:
        i_maxvol, Q_maxvol = maxvol_rect(Q, maxvol_options)
...
def maxvol_sqr(A, maxvol_options):
    """
    ...
    maxvol_options : dict
        A dictionary containing the free parameters of the algorithm.
...
def maxvol_rect(A, maxvol_options):
    """
    ...
    """
    n, r = A.shape
    r_min = r + maxvol_options["min_rank_change"]
    r_max = (
        r + maxvol_options["max_rank_change"]
        if maxvol_options["max_rank_change"] is not None
        else n
    )
    r_max = min(r_max, n)
    if r_min < r or r_min > r_max or r_max > n:
        raise ValueError("Invalid minimum/maximum number of added rows")
    I0, B = maxvol_sqr(A, maxvol_options)
    ...
    for k in range(r, r_max):
        i = np.argmax(F)
        if k >= r_min and F[i] <= maxvol_options["tau"] ** 2:
            break
        I[k] = i
        S[i] = 0
        v = B.dot(B[i])
