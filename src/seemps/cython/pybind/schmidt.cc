...
double
_recanonicalize(py::list state, int oldcenter, int newcenter,
                const Strategy& strategy)
{
  double err = 0.0;
  while (oldcenter < newcenter)
    {
      auto [site, errk] = _update_in_canonical_form_right(
          state, state[oldcenter], oldcenter, strategy);
      err += errk;
      oldcenter = site;
    }
  while (oldcenter > newcenter)
    {
      auto [site, errk] = _update_in_canonical_form_left(
          state, state[oldcenter], oldcenter, strategy);
      err += errk;
      oldcenter = site;
    }
...
double
_canonicalize(py::list state, int center, const Strategy& strategy)
{
  double err = 0.0;
...
_left_orth_2site(py::object AA, const Strategy& strategy)
{
  ...
  auto [U, s, V] = destructive_svd(as_matrix(AA, a * d1, d2 * b));
...
double
_update_canonical_2site_left(py::list state, py::object A, int site,
                             const Strategy& strategy)
{
...
double
_update_canonical_2site_right(py::list state, py::object A, int site,
                              const Strategy& strategy)
{
  if (!is_array(A) || array_ndim(A) != 4)
    {
