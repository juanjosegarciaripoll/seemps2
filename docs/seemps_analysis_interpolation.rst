...
a function :math:`f\left(x_s^{(n)}\right)` on an :math:`n`-qubit system with a discretization step of :math:`\Delta x_s^{(n)}`,
the second-order finite difference interpolant calculates the new points of the :math:`(n+1)`-qubit grid as
...
If the function is bandwidth-limited and for a sufficiently small spacing :math:`\Delta{x}^{(n)}\leqslant 2\pi/L_p`
...
(ii) addition of :math:`m` auxiliary qubits to enlarge the momentum space with qubit reordering :math:`U_\text{sym}` to map the original discretization with :math:`2^n` to the
intervals :math:`s \in [0,2^{n-1}) \cup [2^{n+m}-2^{n-1},2^{n+m})`, and (iii) Fourier transform back to recover the state with :math:`n+m` qubits. The complete algorithm reads
