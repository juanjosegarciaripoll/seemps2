...
to efficiently represent functions and operators in an :math:`n`-qubit quantum register.
By setting :math:`N=2^n`, each of the points :math:`x_s` of the grid maps to a quantum state :math:`s`
...
It is also possible to use a symmetric definition of the position space interval, by setting :math:`\Delta x = L/(N-1)`.

Multidimensional functions
--------------------------

Let us consider a :math:`d`-dimensional function :math:`f(\mathbf{x})=f(x_1, x_2,\dots,x_d)` with a definition interval :math:`x_i \in [a_i,b_i)`
of size :math:`L_{x_i}=|b_i-a_i|` for each dimension :math:`i=(1,\dots,d)`. An :math:`n`-qubit quantum register
...
:math:`i`-th dimension. Then, a set of labels :math:`\mathbf{s}=(s_1,s_2,\dots,s_d)` represents the :math:`d`-dimensional state for the coordinate
...
The :class:`~seemps.analysis.space.Space` class creates an object to define the problem's coordinates for a multidimensional problem.
